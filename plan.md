...
10. **DONE:** Forward metrics aggregation per deployment — `backend/app/forward_metrics.py` computes win-rate, avg P&L, total P&L, profit factor, and excluded-incomplete-session trade counts from closed paper trades. Session completeness is measured on `candles_1m` from 10:00-15:00 IST; >=210/300 minutes is complete. Strategy Library fetches visible metrics and shows them only when `complete_session_count >= 10`.
...
12. **DONE:** Per-deployment kill switches (paper mode) — `backend/app/deployment_kill_switch.py`. `max_consecutive_losses` and `daily_loss_cutoff_pct` auto-PAUSE the deployment (hard circuit-breaker, evaluated right after the drift check, stamps `kill_switch_reason`/`kill_switch_inputs`); `max_open_paper_trades` is a soft BLOCK that adds a signal blocker but leaves the deployment ACTIVE so it self-clears as trades close. Configurable via `DeploymentCreateReq` fields merged into `deployment.risk`; Live Signals form exposes them; deployment card shows the pause reason. 16 unit + 2 evaluator-integration + 1 contract test (306 total passing).
