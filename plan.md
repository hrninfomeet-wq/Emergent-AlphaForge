...
**Next numbered slice is Slice 12 — per-deployment kill switches.** After that, Phase 5 (probability engine) is deferred until ≥6 months of forward signal history exists.
...
12. **NEXT:** Per-deployment kill switches — auto-pause on `max_consecutive_losses`, `daily_loss_cutoff_pct`, `max_open_paper_trades`. User-configurable per deployment.
