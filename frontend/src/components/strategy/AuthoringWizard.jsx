...
export default function AuthoringWizard({ open, onOpenChange, onInstalled }) {
  ...
  // A premium-trigger config generated by the AI. Held verbatim and re-emitted
  // on Install: buildSpec reconstructs the spec from form state, so anything not
  // round-tripped here is silently discarded between generation and install.
  const [premiumTrigger, setPremiumTrigger] = useState(null);
  const isPremiumTrigger = !!premiumTrigger;
  ...
  function buildSpec() {
    ...
    // Re-emit the config verbatim. The compiler REFUSES a spec carrying both a
    // config and entry conditions, so clear those rather than send a spec that
    // cannot install.
    if (premiumTrigger) {
      spec.premium_trigger = premiumTrigger;
      spec.entry_ce = [];
      spec.entry_pe = [];
    }
    return spec;
  ...
  function loadFromSpec(spec) {
    ...
    setPremiumTrigger(spec.premium_trigger || null);
    // Capture the config the AI generated. Without this the whole block is lost
    // the moment it arrives, and the strategy silently installs as an ordinary
    // (non-trading) one.
    setPremiumTrigger(spec.premium_trigger || null);
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) setInstalledId(null); onOpenChange(v); }}>
      ...
      >
        ...
        {/* c. Premium-trigger config — shown INSTEAD of the entry editors.
            A premium-native strategy is driven by the premium session engine,
            which replaces evaluate() entirely, and the compiler REFUSES a spec
            carrying both a config and entry conditions. Rendering editable entry
            rows here would invite the user to build something that cannot
            install. Without this panel a premium strategy shows an EMPTY rules
            list and looks like generation failed. */}
        {isPremiumTrigger ? (
          <div className={sectionCls} data-testid="premium-trigger-panel">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              ...
              <button
                type="button"
                onClick={() => setPremiumTrigger(null)}
                className="text-[10.5px] text-dimmer hover:text-foreground underline"
                data-testid="premium-trigger-clear"
              >
                Convert to a per-bar rule strategy
              </button>
            </div>
            <div className="text-[11px] text-dim leading-snug">
              This strategy does not use per-bar indicator rules. It locks an option
              strike at the reference time and enters when the option premium itself
              moves — run by the premium session engine.
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
              {Object.entries(premiumTrigger)
                .filter(([, v]) => v !== null && v !== undefined && v !== "")
                .map(([k, v]) => (
                  <div key={k} className="rounded border border-line bg-bg-2 px-2 py-1">
                    <div className="text-[9.5px] uppercase tracking-wider text-dimmer">
                      {k.replace(/_/g, " ")}
                    </div>
                    <div className="text-[11.5px] font-mono text-foreground break-all">
