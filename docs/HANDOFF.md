...
Latest (2026-06-13): **backtest results redesign (CHANGELOG 0.31.x)** — decision-first `PerformanceOverview` (rupee hero, account-value+underlying chart with drawdown pane, trade-quality metrics) in `components/backtest/` + pure `lib/backtestMetrics.js`; deep research cards collapsed into Advanced analytics. CAGR/Calmar suppressed <1y (vanity-number guard); headline reward/risk is span-independent Profit÷maxDD. Underlying line is context, NOT a benchmark (user declined). Frontend-only, derived from existing run data. Follow-ups not done (optional): full monolith split into tabs, a shared backend performance-metrics module, monthly P&L calendar.

Latest (2026-06-13, late): **warehouse review follow-ups (CHANGELOG 0.30.x)** — F2 partial-day spot repair (under-captured closed sessions like 2026-06-12 255/375 are now detected via calendar-expected counts and re-fetched + re-banded; verified 255→375, heatmap green), F3 VIX folded into Sync now + the daily auto-update loop, F5 manual planner relabeled as moneyness-selection (not band). IMPORTANT CORRECTION: the option strikes the panel shows as "broker-empty" (NIFTY 25000 PE, SENSEX round strikes, etc.) were RE-VERIFIED as genuinely unavailable at Upstox (one authoritative token each, 0 candles, no duplicate in the 61.7k-contract store) — the W1 ledger is correct; an earlier "wrong-key/re-fetchable" hypothesis was withdrawn. 522 tests pass.
