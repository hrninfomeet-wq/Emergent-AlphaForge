...
- **Request timeouts (done #1; #3 is the target)**: long synchronous calls (backtest run, warehouse sync, data-hygiene scans, audits) now get a **per-request 10-min timeout** via `LONG_TIMEOUT_MS` in `frontend/src/lib/api.js` (global stays 60s; build-time tunable via `REACT_APP_API_TIMEOUT_LONG`). The `"timeout of 60000ms exceeded"` error was the **axios client** timeout, not a backend param — the backend has no request timeout and still saves the run to `backtest_runs` after a client abort (recoverable via "Load past run"). **Future target (#3):** convert `POST /backtest/run` to the optimizer's fire-and-poll job pattern (return `run_id` immediately, run via `asyncio.create_task` + `asyncio.to_thread`, poll the existing `GET /backtest/runs/{id}` through `JobsProvider`). That removes the timeout class entirely **and** stops the synchronous backtest from blocking the uvicorn event loop / 5s healthcheck (today `run_backtest`/`walk_forward` run inline on the loop, unlike the optimizer which already uses `to_thread`). An orphaned-run recovery backstop (surface the completed run after a client timeout) is the medium-effort companion.
