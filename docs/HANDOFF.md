...
- **Unpushed: 7 commits** on `main` (origin at `1c2538c`; the earlier chart commits are already pushed): the **6-slice live-realism/gate-rigor hardening pass** (`4bc7df3` friction, `754ebb2` gate, `24347db` cockpit, `e914de4` exits, `6d4b8d3` manual-safety, `50a0062` chart+rerank) + the `6d6917d` docs commit. Push only on the user's explicit "push" (per-changeset approval).
