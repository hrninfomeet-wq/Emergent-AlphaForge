...
| 10 | Option-side flow (CE/PE volume, OI) does not reach `evaluate()` — blocks Candidate A (§7.1) | M | **DONE 2026-08-28** — 11 columns through the existing `required_data` seam (`app/option_flow.py` + a second source kind in `warehouse.attach_required_data`); no new declaration surface, so the AI authoring stack works unchanged. The 20-session baseline is computed in the DATA LAYER, not the strategy: verified on the real warehouse that a 1,000-bar live frame and a 15,000-bar backtest frame give **identical** values for all 11 columns. Bars joined by identity, never token. 45 tests; mutation sweep 34/34 killed (first sweeps had 14 survivors, and fixing them found a real defect — the fetch had its own copy of the ATM-anchor rule, so the contract QUERIED could drift from the one MATCHED). Three findings in deliverable §15.4: one session (2026-08-27) has zero option bars; **61% of SENSEX same-minute OI-delta baselines are flat**, so `flow_imbalance` is unavailable on ~61% of SENSEX bars; volume z-scores are one-sided (never below −1.5). **Candidate A screened 2026-08-28: REJECTED on both indices** — see item #18. |
| 18 | **Candidate A (ATM Premium-Flow Scalp) — SCREENED and REJECTED** | — | **CLOSED 2026-08-28.** Screened against §4.1's pre-registered kill thresholds on the TRAIN slice only, holdout guard armed and untouched. Conditioned MFE/MAE at 10 min = **0.856 (NIFTY) / 0.703 (SENSEX)**, both must exceed 1.15; session-level t-stat on net% = **−2.81 / −2.24**, both must exceed +2.0. **REJECT on both binding tests, both indices.** In 11 of 12 conditioned cells the ratio is BELOW the unconditioned base rate on the same series — the condition selects worse-than-average bars. All three pre-registered `flow_z_threshold` values {1.0, 1.5, 2.0} fail, and loosening makes it worse. Two checks run before believing the negative: the unconditioned base rate reproduces (0.856–0.900 vs the documented 0.90–0.95, §5.2's gate), and a sign-inversion falsification also fails (0.950, t −2.28), so it is not a wiring error. **No plugin written** — the screen cost hours and built nothing, which is the point. Deliverable §16; probe `backend/scripts/screen_candidate_a_flow.py`. |
| 19 | Candidate A shipped as a plugin ANYWAY, by operator decision | — | **BUILT 2026-08-28, UNVALIDATED.** `atm_premium_flow_scalp`, written after item #18 rejected the hypothesis, at the operator's explicit instruction and to be hand-optimized by them. **It carries no edge claim and its own docstring leads with the rejection.** First strategy in the repo to declare `required_data` (the seven option-flow columns). Frozen §4.1 budget: six searchable dimensions, `signal_threshold` pinned, cooldown/ATR-mult/liquidity-mult are module constants so the search cannot grow. `live_lookback_bars=400` so the session VWAP anchor and the session trade budget are identical live and in backtest. 43 tests; mutation sweep **27/27 killed** (4 survivors on the first pass, one of which — the missing-z-score guard — was covered by both callers and needed a direct unit test). End-to-end on the real engine, NIFTY train window: 174 trades, **PF 0.745, win rate 31.0%, avg −2.01 pts** at defaults, i.e. negative exactly as §16 predicted. `tests/test_data_columns_wiring.py`'s "nothing declares required_data" tripwire was narrowed deliberately rather than deleted. **Do not deploy to real money on current evidence.** |
