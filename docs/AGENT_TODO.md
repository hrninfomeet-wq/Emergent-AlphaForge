...
| C2 | Transmit fence (stale authorization) | 🔄 IN PROGRESS 2026-07-25 | DESIGN: `allow_fn` closes over a STALE deployment doc + FROZEN `now`, so re-calling it is useless. Add an async `recheck_fn` param to `executor.place_deployed_order`, invoked AFTER the armed check and BEFORE the throttle (throttle.allow is sync, so zero await remains before transmit — tightest possible fence; also avoids burning a SEBI rate token on an aborted entry). auto_live supplies it: re-fetch the deployment from Mongo, require status==ACTIVE **and** `is_deployment_live_allowed(fresh, now(), connected)`. NB `pause` only sets status (mode stays "live"), so the status check is load-bearing. Block reason: `stale_authorization:<why>`. recheck_fn=None → skip + log (keeps existing executor unit tests valid); a contract test pins that auto_live always passes it. |
| C | Deferred pre-real-money fixes (H1, C3) | ⏸ DEFERRED | MUST land before first real-money session — §2 |
