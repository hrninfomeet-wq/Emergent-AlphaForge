...
| 5 | New strategy plugins (candidates, honestly validated) | ➡ NEXT | **Scope now set by item 6's ranking, not by "write more strategies".** Do F then B: (F) make VIX/OI/structural features reachable as causal per-bar signal + kill the dead `vix_boost_threshold` knob; (B) pooled-3-index regime routing under the pre-registered criterion in `docs/PROFIT_LEVERAGE_ANALYSIS_2026-07.md` §6 — split by DATE not index, exclude the BANKNIFTY Dec-2024 gap, and report per-index holdout separately (single-index positive = killed). C (realized-fill vs model friction analysis) can run in parallel; needs no engine work. |
