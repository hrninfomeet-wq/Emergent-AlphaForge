...
| 9.1 | Phase 1 — config-block generalization (strategy builder) | ➡ **NEXT** | The builder's real ceiling: `classify_rule` answers BUILDABLE_NOW for premium-trigger / session-gate / sizing / expiry concepts, but `StrategySpec` has no field for them, `compile_spec` never emits them, neither generator prompt mentions them, and dispatch is gated by `if strategy_id != "premium_momentum": return None` — while the docstring directly below states the design was to route on CONFIG PRESENCE. The app promises buildable and then cannot build. Completion of specified design, not new invention. Plan: `docs/CAPABILITY_PHASE_PLAN_2026-07.md` §Phase 1. |
