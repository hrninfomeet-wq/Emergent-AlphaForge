...
| C3 | Account-global caps | 🔄 IN PROGRESS 2026-07-27 | DESIGN v1: new `check_account_caps()` in `live_deploy_governor.py` queries `live_trades` with NO deployment filter → account-wide open count + today's realized+unrealized, then delegates the verdict to the EXISTING pure `evaluate_guardrails(mtm, open_count, config)` so account semantics live in one place (it already implements loss-limit / max-open / profit-lock with a fail-safe on non-finite inputs). Refuses as `account_<action>`; on `broker_stop_loss` it also calls `engine.guardrail_tick(...)` — which trips the safety latch + halts the engine, finally giving `guardrail_tick` its production caller. Also honours the `blocked_until_reset` latch via `is_entry_blocked`. Wired into auto_live right after the per-deployment governor: BOTH must pass. v2 (atomic reserve-then-place) deferred — single-deployment usage makes v1 the priority. |
| C | (was: deferred pre-real-money fixes — C2/H1 done, C3 in progress) | ✅ CLEARING | MUST land before first real-money session — §2 |
