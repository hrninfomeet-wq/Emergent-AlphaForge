...
| 5 | New strategy plugins (candidates, honestly validated) | 🔄 5F IN PROGRESS | **Scope now set by item 6's ranking, not by "write more strategies".** Do F then B. **F progress 2026-07-27:** (i) the dead `vix_boost_threshold` knob is **KILLED** (`e165d63`, pushed) — `explosive_reversal` read a `vix` column no engine path builds; removed the knob + branch and added `tests/test_strategy_column_contract.py`, a general net that fails ANY strategy reading a column the engine does not build (an AST sweep found `vix` was the only phantom column in all 12 strategies). Sibling cleanup in `dee517d`: `classify_scenario`'s inert `atr_ratio`/`vix_bucket` args + the `_atr_ratio` helper that existed only to feed one of them. (ii) **The VIX data cliff is CLOSED** — INDIAVIX started 2025-06-09 vs spot 2024-11-25, which would have made a per-bar VIX knob live for 68% of the window and dead for the first 32%; backfilled via two `POST /api/warehouse/vix/ingest` calls (49,878 candles / 133 sessions, 0 failed chunks). VIX now spans 2024-11-25→2026-07-24, **412/413 NIFTY sessions (99.8%)** — the one gap is the current day, and the single 60-bar session (2025-10-21) is a real Muhurat half-day confirmed across all 3 instruments. (iii) **REMAINING for F:** the per-bar join itself. NB VIX **cannot** be a `FeatureGroup` — `compute` is `(DataFrame, dict)` pure-pandas by contract while VIX is in Mongo — so it needs a load-time join at ~6 frame-origination sites (`runtime.py:1510`, `routers/research.py:213`+`:299`, `optimizer.py:1087`/`:1116`, `deployment_evaluator.py:441`); `vix_asof` is already at-or-before so causality is free. Also flip `capability.py:27 has_vix_history: False` (provably wrong) **as part of** that change, not before. Then (B) pooled-3-index regime routing under the pre-registered criterion in `docs/PROFIT_LEVERAGE_ANALYSIS_2026-07.md` §6 — split by DATE not index, exclude the BANKNIFTY Dec-2024 gap, and report per-index holdout separately (single-index positive = killed). C (realized-fill vs model friction analysis) can run in parallel; needs no engine work. |
