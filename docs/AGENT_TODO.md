...
| 6 | Profit-leverage ideas write-up | 🔄 IN PROGRESS 2026-07-27 (`dc601df`) | **User chose "item 6 → then 5"**: write the analysis FIRST, then build only the top idea as item 5 with a pre-declared kill criterion — avoids repeating the premium-momentum hunt. Deliverable `docs/PROFIT_LEVERAGE_ANALYSIS_2026-07.md`. **Core finding: the app is LONG-PREMIUM ONLY by construction** — `direction: "CE"\|"PE"\|"NONE"` with no side (`strategies/base.py:21`), `pnl_pts = exit - entry` (`option_backtest.py:749`), `side="B"` always (`auto_live.py:483`); zero short-side vocabulary repo-wide; `option_costs.py:3` says "for an options-buying strategy". So all three failed campaigns searched inside ONE family whose defining property is PAYING the variance premium. Their own holdout measured −798 gross points BEFORE friction over 128 untouched sessions. Counter-argument stated up front: the mirror of a losing long is NOT automatically a winning short (two-sided friction; short gamma inverts into the many-small-wins/rare-ruin shape that flatters a 20-month backtest; margin replaces premium outlay; risk machinery assumes max-loss = premium paid) → **defined-risk spreads only, tail-aware kill criterion**. Directions A–E ranked by information-per-build. Awaiting 3 Sonnet inventory agents (short-side change surface; warehouse coverage incl. whether spread strikes even exist; computed-but-not-strategy-visible signals). |
