...
| **Phase 2** — remove the remaining friction | ✅ **COMPLETE 2026-08-01.** Spot-data Check → shared Ingest → auto-recheck; async error/audit parity; optimizer cancel honesty (#17/#20); operator navigation; no-secret `FERNET_KEY` boot warning; either-IP recognition and exact static-IP guidance. ⚠ **CORRECTED 2026-08-15:** `PositionMonitor.jsx` is NOT wired — `LiveCockpit.jsx:19-24` mounts six panels and this is not one of them, and `LiveDataProvider.jsx:9` mentions it only in a comment. It is an L2-era manual **test-order** panel superseded by `KillSwitchPanel`; deployed positions exit via the software guard, the OCO backstop, the 15:00 EOD square and the kill switch, so no deployed-position exit path is affected. Still undecided: wire it or delete it (see line ~158). |
