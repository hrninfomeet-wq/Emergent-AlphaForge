...
- **Residual risk accepted:** once installed, a custom plugin's `evaluate()` runs **in-process** during paper/backtest like any other plugin. The pipeline guards *install*, not *ongoing evaluate() execution*. Acceptable for a single-user local tool with code review. **The audit established that the more dangerous risk — arbitrary code at IMPORT time, executed in-process by the install reload — is eliminated by the §4.2 module-top-level allowlist (imports run, but no module/class-level statements execute).** **Security posture (honest):** pure-AST `static_check` is **defense-in-depth that prevents accidental dangerous code**, NOT a perfect jail — a second red-team proved categorical bypasses (runtime-built string method-dispatch like `df.agg(s)`, and the general impossibility of statically sandboxing arbitrary Python). The **authoritative control is the mandatory human review** of the generated code in the editable panel before Install. Crucially, the **baseline is that the existing plugin system already imports and runs arbitrary user `.py` strategies in-process** (auto_discover) — Part 2 adds AI generation PLUS a static gate PLUS a subprocess smoke-test PLUS forced human review on top, so it is **strictly safer than hand-writing a plugin file**. Deploy-time sandboxing of `evaluate()` remains out of scope.
...
5. **Whole-tree denylist** (`ast.walk`, applies inside methods too):
   ...
   - any `Attribute` whose `.attr` starts with `_` → reject (subsumes the dunder regex; closes private re-export walks like `pd._testing._os`, `np._globals`). [red-team hardening]
   - any `Attribute` whose `.attr` starts with `read_` or is in `FORBIDDEN_ATTR_NAMES` (pandas/numpy I/O `read_*`/`to_pickle`/`to_csv`/…, string-exec `eval`/`query`, numpy `load`/`save`/`fromfile`/`tofile`/`memmap`, process `system`/`popen`/`run`/…) → reject. [red-team hardening]
   - any `Call` whose func is an `Attribute` with `.attr` in `FORBIDDEN_CALL_NAMES` (not just bare Names) → reject (closes `blt.eval(...)`). [red-team hardening]
   - method `decorator_list` non-empty, or method default-arg / kw-default values that aren't literals → reject (closes class-definition/import-time execution via default-arg evaluation). [red-team hardening]
   - the module must contain `from __future__ import annotations` → required, so parameter annotations never execute at def time. [red-team hardening]
...
## 7. Risks & mitigations
- **AST evasion (acknowledged limit):** two adversarial red-team Workflows found 36 evasions total; the accident-prevention ones (private re-export, attribute-call eval, pandas/numpy I/O incl. `loadtxt`/`savetxt`/`genfromtxt`, import-time default-arg execution, bare-Name aliasing, literal string-dispatch) are CLOSED + regression-tested. Two classes are categorical and NOT closed: a runtime-built string passed to `.agg`/`.apply`/`.eval` (the AST can't resolve a computed string), and the general undecidability of statically proving arbitrary Python is side-effect-free. These are accepted because (a) the threat model is accidental AI mis-generation, not a crafted local attacker; (b) the user reviews the code before install; (c) the existing plugin system already runs arbitrary user Python in-process. The static gate raises the bar against accidents; the human review is the gate against intent.
